...
public class GroundPlaneUI : MonoBehaviour
{
    ...
    PlaneManager PlaneManagerObject;
    ...
    public GameObject Show3DContentBtn;

    public GameObject[] CanvasItems;
    public GameObject[] ThreeDGameObjects;
    public GameObject ThreeDContentPanel;
    bool ThreeDContentIsActive = false;

    #region MONOBEHAVIOUR_METHODS
    void Start()
    {
        PlaneManagerObject = FindObjectOfType(typeof(PlaneManager)) as PlaneManager;
        ...
        Switch3DContent(false);
        OpenClose3DContentPanel(false);
    }
    ...
    bool ThreeDContentPanelIsActive;

    public void OpenClose3DContentPanel()
    {
        if (PlaneManager.instance.DeviceNotSupported())
        {
            MessageBox.DisplayMessageBox("Menta AR", " Device not supported for head modelling ", true, null);
            return;
        }

        if (!ThreeDContentPanelIsActive)
        {
            ThreeDContentPanelIsActive = !ThreeDContentPanelIsActive;
            ThreeDContentPanel.SetActive(ThreeDContentPanelIsActive);
            Show3DContentBtn.SetActive(!ThreeDContentPanelIsActive);
        }
        else
        {
            Switch3DContent();
    ...
    public void OpenClose3DContentPanel(bool val)
    {
        ThreeDContentPanelIsActive = false;
        ThreeDContentPanel.SetActive(val);
        Show3DContentBtn.SetActive(!val);
    }

    public void Switch3DContent()
    {
        ThreeDContentIsActive = !ThreeDContentIsActive;
        foreach (GameObject G in ThreeDGameObjects)
        {
            G.SetActive(ThreeDContentIsActive);
        }
        ...
        ThreeDContentPanelIsActive = ThreeDContentIsActive;
        ThreeDContentPanel.SetActive(false);
        Show3DContentBtn.SetActive(true);


        Show3DContentBtn.GetComponent<Image>().sprite = ThreeDContentIsActive ? CloseIcon : HeadIcon;

        Invoke("TurnOnMidAir", 1);
    }

    void TurnOnMidAir()
    {
        m_MidAirToggle.onValueChanged.Invoke(true);
    }

    public void Switch3DContent(bool val)
    {
        foreach (GameObject G in ThreeDGameObjects)
        {
            G.SetActive(val);
        ...
        Show3DContentBtn.GetComponent<Image>().sprite = val == true ? CloseIcon : HeadIcon;
    }
